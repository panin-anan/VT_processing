...
try:
    for m_idx in range(Nm):
        phi = np.real(mode_shapes[:, m_idx]).astype(float)
        ux = np.zeros_like(phi)
        uy = np.zeros_like(phi)
        uz = phi

        ds2420 = pyuff.prepare_2420(
            id1=f"{test_set_name}",
            id2="Modal Test (2420)",
            id3=f"Mode {m_idx+1}",
            id4="",
            # SDRL/UNV 2420 header
            result_type=1,        # nodal DOF results
            model_type=1,         # structural
            analysis_type=2,      # normal modes
            data_type=2,          # Real
            step=float(frequencies[m_idx]),  # many readers use 'step' as frequency for modes
            subcase=1,            # this helps NX group as a subcase
            # per-node values (3-translation vector)
            node_nums=node_ids.astype(int),
            r1=ux, r2=uy, r3=uz
        )
        uff_sets.append(ds2420)
except Exception:
    # If prepare_2420 isn't available, DS-58 alone is sufficient for NX to create a test subcase.
    pass
